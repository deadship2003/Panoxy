...
7. **离线包内核按「打包机」CPU 选型**:`build.sh package` 在打包时探测本机 AVX2 决定内置 v3/标准档内核;跨 CPU 类别部署(有 AVX2 的机器打包 → 无 AVX2 的老机器)会跑不起来。目标机用 `Panoxy init` 或 `Panoxy upgrade` 重新探测下载即可(`deploy` 不探测,直接解包内置内核)
